...
def calculate_totals(pc_data):
    ...
    for unit,unit_wells in enumerate(pc_data):
        ...
        unit_total_qwat=0.0 # initialise unit totals for Qoil
        ...
        for well,val in unit_wells.items():
            ...
            if val["qwat"]:
                unit_total_qwat+=float(val["qwat"])
        ...
        for rms in rmss: # loop through RMSs
            ...
            for well,val in pc_data[unit].items(): # loop through wells
                ...
                if curr_route_dict["rms"] == rms: # if in this RMS, then add to RMS total
                    ...
                    if val["qwat"]:
                        totwat+=float(val["qwat"])
            subtotals[rms]={"qoil":totoil,"qgas":totgas,"qwat":totwat}

        # put subtotals and unit totals into totals dictionary
        unit_totals.append({"subtotals":subtotals,"qgas":unit_total_qgas,"qoil":unit_total_qoil,"qwat":unit_total_qwat})
    ...
    field_wat=unit_totals[0]["qwat"]+unit_totals[1]["qwat"]+unit_totals[2]["qwat"]

    totals={
        "field":{"qgas":field_gas,"qoil":field_oil,"qwat":field_wat},
        "unit":unit_totals
    }
...
def merge_ref(data):
    ...
    for i,d in enumerate(data["totals"]["unit"]):
        for key,val in d["subtotals"].items():
            if key in data_ref["totals"]["unit"][i]["subtotals"]:
                ...
                data["totals"]["unit"][i]["subtotals"][key]["qwat_ref"]=data_ref["totals"]["unit"][i]["subtotals"][key]["qwat"]
    ...
    for i,d in enumerate(data["totals"]["unit"]):
        ...
        data["totals"]["unit"][i]["qwat_ref"]=data_ref["totals"]["unit"][i]["qwat"]


    data["totals"]["field"]["qgas_ref"]=data_ref["totals"]["field"]["qgas"]
    data["totals"]["field"]["qoil_ref"]=data_ref["totals"]["field"]["qoil"]
    data["totals"]["field"]["qwat_ref"]=data_ref["totals"]["field"]["qwat"]
